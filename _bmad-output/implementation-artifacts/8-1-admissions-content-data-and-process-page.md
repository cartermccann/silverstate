...
- [x] **Task 1: Create `src/data/admissions.ts` — typed content data** (AC: #1)
  - [x] 1.1: Import `AdmissionStep` and `FaqEntry` from `../types`. Uses `AdmissionStep` (singular) — matching the existing interface name in `types.ts` from the mockup. Do NOT create an alias or use `AdmissionsStep`.
  - [x] 1.2: Export `admissionsSteps: AdmissionStep[]` — a typed array of 4 admissions process steps:
  ...
  - [x] 1.3: Each step must include at minimum: `title: string`, `desc: string` (the existing `AdmissionStep` interface uses `desc`, not `description` — this is grandfathered). Verify the actual `AdmissionStep` interface shape from `types.ts` and match it exactly
  - [x] 1.4: Export `admissionsFaqs: FaqEntry[]` — a typed array of 6-8 FAQ entries covering common admissions questions:
  ...
  - [x] 1.5: Export `admissionsPageMeta` object with `title`, `description`, and `ogImage` for SEO metadata generation
  - [x] 1.6: All exports must have explicit type annotations — never `export const admissionsSteps = [...]` without the type
  - [x] 1.7: Add re-export of admissions data to `src/data/index.ts` barrel file

- [x] **Task 2: Create Admissions page at `src/pages/admissions/Process.tsx`** (AC: #2, #3, #5, #6, #8)
  - [x] 2.1: Create the file at `src/pages/admissions/Process.tsx` following the Architecture directory structure
  - [x] 2.2: Use `export default function Process()` — not arrow function export
  - [x] 2.3: Import `admissionsSteps` and `admissionsFaqs` from `../../data/admissions`
  - [x] 2.4: Import `site` from `../../data/common` for phone number — never hardcode
  - [x] 2.5: Import PageLayout wrapper or ensure the route config wraps this page in PageLayout (depends on how Story 1.7 implements layout wrapping — if PageLayout is a route-level wrapper in `routes.ts`, the page does not import it directly)
  - [x] 2.6: Render a page title as `<h1>` — e.g., "Admissions Process" or "How to Get Started"
  - [x] 2.7: Render the step-by-step process section with numbered steps. Each step should display: step number, step title, and step description. Use the existing `StepCard` component if its props match, or render a custom numbered list section. Semantic HTML: use an `<ol>` for the ordered process or `<section>` elements with clear visual numbering
  - [x] 2.8: Render the FAQ section using `FaqItem` component (from `src/components/FaqItem.tsx`) for each FAQ entry. Wrap in an FAQ section with an `<h2>` heading
  - [x] 2.9: Render a prominent phone CTA section with 24/7 messaging — e.g., "Ready to take the first step? Our admissions team is available 24/7." with a large `<a href={site.phoneTel}>` button using the `.btn` class
  - [x] 2.10: Render internal links section connecting to:
  ...
  - [x] 2.11: Use `<Link to="...">` from React Router for all internal navigation links — never `<a href>` for internal routes

- [x] **Task 3: Implement SEO metadata and JSON-LD** (AC: #4, #7)
  - [x] 3.1: Use `generateMeta()` from `utils/meta.ts` to produce the route `meta` export with title (e.g., "Admissions Process | Silver State Adolescent Treatment Center"), description, canonical URL (`/admissions`), and OG tags
  - [x] 3.2: Use `generateFAQPage()` from `utils/schema.ts` to produce FAQPage JSON-LD from `admissionsFaqs`
  - [x] 3.3: Include the JSON-LD in the page's `<head>` via the route `meta` export or a `<script type="application/ld+json">` tag in the component body
  - [x] 3.4: Verify the FAQPage JSON-LD includes all FAQ question/answer pairs

- [x] **Task 4: Ensure route is configured** (AC: #2)
  - [x] 4.1: Verify `/admissions` route exists in `src/routes.ts` and maps to `pages/admissions/Process.tsx`
  - [x] 4.2: If the route does not exist, add it following the existing route pattern in `routes.ts`

- [x] **Task 5: Responsive layout and accessibility** (AC: #8)
  - [x] 5.1: Use `.wrap` container for max-width centering (1200px)
  - [x] 5.2: On mobile (< 900px), all content stacks vertically in a single column
  - [x] 5.3: Heading hierarchy: `<h1>` for page title, `<h2>` for section headings (Process Steps, FAQ, Contact), `<h3>` for individual step titles if needed
  - [x] 5.4: All interactive elements (links, phone CTA) have visible focus indicators via global `:focus-visible` styles
  - [x] 5.5: Phone CTA touch target meets 44x44px minimum on mobile
  - [x] 5.6: All colors use CSS tokens — `var(--blue)`, `var(--text)`, `var(--body)`, `var(--cream)`, etc.
  - [x] 5.7: No horizontal scrolling at any viewport width (FR43)

- [x] **Task 6: Verify compilation and rendering** (AC: all)
  - [x] 6.1: Run `npx tsc --noEmit` — zero TypeScript errors
  - [x] 6.2: Run `npm run dev` — navigate to `/admissions`, page renders correctly
  - [x] 6.3: Verify all 4 steps display with correct content
  - [x] 6.4: Verify FAQ section renders and items expand/collapse
  - [x] 6.5: Verify phone CTA is visible and links correctly
  - [x] 6.6: Verify internal links navigate to correct routes
  - [x] 6.7: Check at 320px, 768px, and 1024px viewports — no overflow, all content visible
