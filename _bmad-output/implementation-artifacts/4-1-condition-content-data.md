...
- [x]**Task 1: Replace the placeholder `src/data/conditions.ts` with full condition data** (AC: #1, #4)
  - [x]1.1: Remove the placeholder empty array from `data/conditions.ts` created by Story 1.2
  - [x]1.2: Import `ConditionData`, `FaqEntry`, and `SourceCitation` types from `../types`
  - [x]1.3: Export a `conditions` constant typed as `ConditionData[]` containing all 25 condition objects
  - [x]1.4: Preserve any legacy re-export from the mockup's `content.ts` if Story 1.2 set one up (e.g., `conditionNames` string array for homepage use) -- do NOT remove it; add the full data alongside it

- [x]**Task 2: Create Mental Health condition entries (~12)** (AC: #1, #2, #3)
  - [x]2.1: `anxiety` -- slug: `anxiety-treatment`, category: `mental-health`, name: `Anxiety Treatment`
  - [x]2.2: `depression` -- slug: `depression-treatment`, category: `mental-health`, name: `Depression Treatment`
  - [x]2.3: `trauma-ptsd` -- slug: `trauma-ptsd-treatment`, category: `mental-health`, name: `Trauma & PTSD Treatment`
  - [x]2.4: `suicidal-ideation` -- slug: `suicidal-ideation-treatment`, category: `mental-health`, name: `Suicidal Ideation Treatment`
  - [x]2.5: `ocd` -- slug: `ocd-treatment`, category: `mental-health`, name: `OCD Treatment`
  - [x]2.6: `bipolar-disorder` -- slug: `bipolar-disorder-treatment`, category: `mental-health`, name: `Bipolar Disorder Treatment`
  - [x]2.7: `autism-spectrum` -- slug: `autism-spectrum-treatment`, category: `mental-health`, name: `Autism Spectrum Treatment`
  - [x]2.8: `oppositional-defiant` -- slug: `oppositional-defiant-disorder-treatment`, category: `mental-health`, name: `Oppositional Defiant Disorder Treatment`
  - [x]2.9: `conduct-disorder` -- slug: `conduct-disorder-treatment`, category: `mental-health`, name: `Conduct Disorder Treatment`
  - [x]2.10: `dmdd` -- slug: `dmdd-treatment`, category: `mental-health`, name: `DMDD Treatment`
  - [x]2.11: `bpd` -- slug: `bpd-treatment`, category: `mental-health`, name: `Borderline Personality Disorder Treatment`
  - [x]2.12: `adjustment-disorder` -- slug: `adjustment-disorder-treatment`, category: `mental-health`, name: `Adjustment Disorder Treatment`

- [x]**Task 3: Create Substance Abuse condition entries (~8)** (AC: #1, #2, #3)
  - [x]3.1: `dual-diagnosis` -- slug: `dual-diagnosis-treatment`, category: `substance-abuse`, name: `Dual Diagnosis Treatment`
  - [x]3.2: `substance-abuse` -- slug: `substance-abuse-treatment`, category: `substance-abuse`, name: `Substance Abuse Treatment`
  - [x]3.3: `alcohol-abuse` -- slug: `alcohol-abuse-treatment`, category: `substance-abuse`, name: `Alcohol Abuse Treatment`
  - [x]3.4: `opioid-abuse` -- slug: `opioid-abuse-treatment`, category: `substance-abuse`, name: `Opioid Abuse Treatment`
  - [x]3.5: `benzodiazepine-abuse` -- slug: `benzodiazepine-abuse-treatment`, category: `substance-abuse`, name: `Benzodiazepine Abuse Treatment`
  - [x]3.6: `cocaine-abuse` -- slug: `cocaine-abuse-treatment`, category: `substance-abuse`, name: `Cocaine Abuse Treatment`
  - [x]3.7: `meth-abuse` -- slug: `meth-abuse-treatment`, category: `substance-abuse`, name: `Methamphetamine Abuse Treatment`
  - [x]3.8: `cannabis-abuse` -- slug: `cannabis-abuse-treatment`, category: `substance-abuse`, name: `Cannabis Abuse Treatment`

- [x]**Task 4: Create Eating Disorder condition entries (~5)** (AC: #1, #2, #3)
  - [x]4.1: `anorexia-nervosa` -- slug: `anorexia-nervosa-treatment`, category: `eating-disorders`, name: `Anorexia Nervosa Treatment`
  - [x]4.2: `bulimia-nervosa` -- slug: `bulimia-nervosa-treatment`, category: `eating-disorders`, name: `Bulimia Nervosa Treatment`
  - [x]4.3: `binge-eating` -- slug: `binge-eating-disorder-treatment`, category: `eating-disorders`, name: `Binge Eating Disorder Treatment`
  - [x]4.4: `arfid` -- slug: `arfid-treatment`, category: `eating-disorders`, name: `ARFID Treatment`
  - [x]4.5: `osfed` -- slug: `osfed-treatment`, category: `eating-disorders`, name: `OSFED Treatment`

- [x]**Task 5: Populate each condition entry with required content fields** (AC: #2, #3)
  - [x]5.1: `description` -- 2-3 paragraphs describing the condition as it manifests in adolescents (ages 11-17). Write in empathetic, parent-facing language -- not clinical jargon. Must reference the fact that Silver State specializes in adolescent treatment
  - [x]5.2: `headline` -- Page headline, e.g., "Adolescent Anxiety Treatment at Silver State"
  - [x]5.3: `symptoms` -- Array of 5-8 symptom strings specific to how this condition presents in teens. Focus on observable behavioral signs parents would notice (e.g., "Withdrawal from friends and activities they used to enjoy" rather than "anhedonia")
  - [x]5.4: `therapies` -- Array of 3-5 evidence-based therapy name strings used at Silver State for this condition (e.g., `['Cognitive Behavioral Therapy (CBT)', 'Dialectical Behavior Therapy (DBT)', 'EMDR']`). Must be real, evidence-based therapies appropriate for the condition
  - [x]5.5: `approach` -- 1-2 paragraphs describing how Silver State specifically treats this condition. Reference the levels of care (Residential, PHP, IOP), the 4:1 staff-to-client ratio, on-site academics, and family involvement
  - [x]5.6: `faqs` -- Array of 3-5 `FaqEntry` objects with parent-focused Q&As. Questions should match real search intent (e.g., "How long does adolescent anxiety treatment take?", "Does insurance cover teen depression treatment?")
  - [x]5.7: `relatedPrograms` -- Array of program slug strings: select from `['residential-treatment', 'php', 'iop']` -- include all that are clinically relevant
  - [x]5.8: `relatedConditions` -- Array of 2-4 related condition slug strings. For example, anxiety's related conditions might include `['depression-treatment', 'trauma-ptsd-treatment', 'ocd-treatment']`
  - [x]5.9: `metaTitle` -- SEO title tag for the condition page. Target under 60 characters total including the ` | Silver State` suffix. Example: `'Adolescent Anxiety Treatment | Silver State'`
  - [x]5.10: `metaDescription` -- 150-160 character SEO meta description. Must include the condition name, "adolescent treatment," and "Silver State" or "Las Vegas"

- [x]**Task 6: Add clinical reviewer attribution to every entry** (AC: #2)
  - [x]6.1: Set `reviewedBy` to `'Dr. Russ Park, MD'` on all condition entries (Silver State's medical director -- this is the named clinical reviewer per FR15)
  - [x]6.2: Set `reviewDate` to a recent date string (e.g., `'2026-02-01'`) on all entries

- [x]**Task 7: Add source citations to every entry** (AC: #3)
  - [x]7.1: Each condition must have 2-4 `SourceCitation` objects in the `sources` array
  - [x]7.2: Citations must reference real, authoritative sources appropriate to the condition:
  ...
  - [x]7.3: Each citation must have a `label` (display text) and `url` (valid URL to the authoritative source)
  - [x]7.4: Example format:
    ```typescript
    sources: [
...
- [x]**Task 8: Add helper exports for category filtering** (AC: #4)
  - [x]8.1: Export `mentalHealthConditions` typed as `ConditionData[]` -- filtered from `conditions` where `category === 'mental-health'`
  - [x]8.2: Export `substanceAbuseConditions` typed as `ConditionData[]` -- filtered from `conditions` where `category === 'substance-abuse'`
  - [x]8.3: Export `eatingDisorderConditions` typed as `ConditionData[]` -- filtered from `conditions` where `category === 'eating-disorders'`
  - [x]8.4: Export a `getConditionBySlug` helper function: `export function getConditionBySlug(slug: string): ConditionData | undefined` that returns the condition matching the given slug

- [x]**Task 9: Verify data integrity** (AC: #5)
  - [x]9.1: Run `npx tsc --noEmit` -- zero TypeScript errors
  - [x]9.2: Verify all 25 condition entries are present by checking `conditions.length === 25`
  - [x]9.3: Verify every entry has a non-empty `description`, `symptoms` array with at least 5 items, `therapies` array with at least 3 items, `faqs` array with at least 3 items, and `sources` array with at least 2 items
  - [x]9.4: Verify no duplicate slugs exist in the array
  - [x]9.5: Verify all `relatedPrograms` slugs reference valid program slugs (`residential-treatment`, `php`, `iop`)
  - [x]9.6: Verify all `relatedConditions` slugs reference valid condition slugs that exist in the `conditions` array
  - [x]9.7: Verify the `data/index.ts` barrel re-exports the new named exports from `conditions.ts`
