...
- [x] **Task 1: Extend `ProgramData` interface in `src/types.ts` for full program page support** (AC: #1, #3)
  - [x] 1.1: Review the existing `ProgramData` interface in `types.ts` — the mockup version has: `label`, `title`, `body`, `features: string[]`, `stat?: string`. This is insufficient for full program pages
  - [x] 1.2: Extend `ProgramData` (or create a new `ProgramPageData` interface that extends it) to include all fields needed for program pages:
    ```typescript
    ...
    ```
  - [x] 1.3: Keep the existing `ProgramData` interface intact for backward compatibility with the homepage data (Story 2.1/2.2) — the homepage uses the simpler interface. The full `ProgramPageData` is for dedicated program pages

- [x] **Task 2: Create or extend `TherapyModality` interface in `src/types.ts`** (AC: #2, #3)
  - [x] 2.1: Define a `TherapyModality` interface in `types.ts`:
    ```typescript
    ...
    ```
  - [x] 2.2: This interface supports both the therapy listings on program pages and potential future standalone therapy description sections

- [x] **Task 3: Create `src/data/programs.ts` with full program page data** (AC: #1, #4)
  - [x] 3.1: Create `src/data/programs.ts` with typed named exports
  - [x] 3.2: Export `residentialProgram` as `ProgramPageData` with complete content:
  ...
  - [x] 3.3: Export `phpProgram` as `ProgramPageData` with complete content:
    - slug: `'php'`
  ...
  - [x] 3.4: Export `iopProgram` as `ProgramPageData` with complete content:
    - slug: `'iop'`
    ...
    - Related conditions and programs populated appropriately
  - [x] 3.5: Export a convenience lookup: `export const programsBySlug: Record<string, ProgramPageData>` mapping slug to data object — enables easy lookup by route parameter
  - [x] 3.6: Export `allPrograms: ProgramPageData[]` as an ordered array (Residential, PHP, IOP) for iteration

- [x] **Task 4: Create `src/data/therapies.ts` with therapy modality data** (AC: #2)
  - [x] 4.1: Create `src/data/therapies.ts` with typed named exports
  - [x] 4.2: Export `therapyModalities` as `TherapyModality[]` containing all evidence-based therapies:
    ...
    - Holistic Treatment
  - [x] 4.3: Each therapy must have a meaningful `description` (2-3 sentences about what the therapy involves) and `howItHelps` (1-2 sentences about adolescent-specific benefits)
  - [x] 4.4: `usedFor` arrays should cross-reference condition slugs that will be defined in Epic 4 (e.g., CBT is used for anxiety, depression, OCD; EMDR is used for trauma/PTSD)
  - [x] 4.5: Export a convenience lookup: `export const therapyBySlug: Record<string, TherapyModality>` mapping slug to data object

- [x] **Task 5: Add re-exports to `src/data/index.ts`** (AC: #3)
  - [x] 5.1: Add `export * from './programs'` to `src/data/index.ts`
  - [x] 5.2: Add `export * from './therapies'` to `src/data/index.ts`

- [x] **Task 6: Build-time validation** (AC: #5)
  - [x] 6.1: Verify `npx tsc --noEmit` passes with zero errors — all type annotations match interface definitions
  - [x] 6.2: If `scripts/validate-content.ts` is available (Story 1.9), verify it validates the new data files:
  ...
  - [x] 6.3: If the validation script is not yet available, add a `// TODO: Validate with scripts/validate-content.ts when Story 1.9 is complete` comment in the data files
...
- Updated `ProgramPageData` interface in `types.ts`: added `metaTitle`, `heroImage?`, renamed `body` to `overview` for semantic clarity (field was unused — `programPages` was empty)
- Updated `TherapyModality` interface in `types.ts`: replaced `abbreviation?` with `shortName`, `relatedConditions?` with `usedFor`, removed `approach?` and `relatedPrograms?`, added `howItHelps` and `evidenceBasis?` per story spec
- Populated `programs.ts` with 3 full `ProgramPageData` objects: `residentialProgram`, `phpProgram`, `iopProgram` — each with complete content including per-program daily schedules, FAQs, therapy modality cross-references, and SEO metadata
- Added convenience exports: `allPrograms` (ordered array), `programsBySlug` (Record lookup), `programPages` (legacy alias for validation script)
- Populated `therapies.ts` with 17 `TherapyModality` objects covering all evidence-based therapies: CBT, DBT, EMDR, TF-CBT, Somatic Experiencing, MI, Family, Group, Individual, Art & Music, Adventure, Meditation, CPI, Medication Management, LGBTQIA+ Affirming Care, Holistic, Trauma-Informed Care
